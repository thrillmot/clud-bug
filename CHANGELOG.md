...
### Added
- **OG / Twitter card image at `/opengraph-image`** (and `/twitter-image`). Generated by `next/og` at the edge with the field-guide composition: Plate № I, scuttling bug, leaf-green title accent, install pill. Restored `summary_large_image` Twitter card now that we have an image.

### Fixed
- **Paragraph indent inconsistency on cludbug.dev.** Removed the `text-indent: 1.4em` rule on `.section-prose p + p` — single-paragraph sections rendered noticeably differently from multi-paragraph ones. Margin-only separation reads consistently across all section variants.
